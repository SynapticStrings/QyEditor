defmodule QyMusic.Note do
  ...
  * `same?/2`：比较两个音符是否相同
  ...
  defdelegate format(note, opts \\ []), to: Note.Format
  ...
  def higher?(note1, note2) do
    do_higher?(format(note1, preference: :sharp), format(note2, preference: :sharp))
  end
  ...
  """
  @spec octive_operate(base_note_and_pitch :: note_and_frq(), target_note :: note()) ::
          note_and_frq()
