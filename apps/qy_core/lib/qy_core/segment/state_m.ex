defmodule QyCore.Segment.StateM do
  ...
  1. 手动参数更新后与此前的参数进行比较，返回需要通过调用模型而更新的参数
  2. 待附带着要更新的参数的更新请求到来时，调用对应的模型进行更新[^why_multi_side]

  [^why_multi_side]: 这么设计是考虑到了一个情形，用户一定会修改模型生成的内容，因此不再需要计算依赖于用户修改
  的参数的那些模型。但是具体的设置以及策略应该交给下游模型去判断，默认是会执行所有的生成任务的。
